...
- **CSP estrita promovida a ENFORCE** (TAREFA 4) — após Report-Only validado limpo em produção (Chrome headless: zero violação em `/pt`, `/en`, `/privacidade`; nonce HTML↔header casando), o middleware passou `Content-Security-Policy-Report-Only` → `Content-Security-Policy` e a CSP loose (Report-Only, `unsafe-inline`) foi removida do `next.config.mjs`. Validado em produção sob enforce: nada bloqueado, GA carrega, demais headers de segurança mantidos. Rollback: `RESPONSE_CSP_HEADER` volta a Report-Only no middleware.
